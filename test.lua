...
local TimeFreezeButton = Instance.new("TextButton")
...
-- Настройка кнопки временной заморозки времени
TimeFreezeButton.Parent = MainFrame
TimeFreezeButton.BackgroundColor3 = Color3.new(0.3, 0.3, 0.3)
TimeFreezeButton.Position = UDim2.new(0.5, -50, 0, 320)
TimeFreezeButton.Size = UDim2.new(0, 100, 0, 50)
TimeFreezeButton.Text = "Freeze Time"
TimeFreezeButton.TextColor3 = Color3.new(1, 1, 1)
TimeFreezeButton.Font = Enum.Font.SourceSans
TimeFreezeButton.TextSize = 24
...
local UICornerTimeFreeze = UICorner:Clone()
UICornerTimeFreeze.Parent = TimeFreezeButton
...
local UIStrokeTimeFreeze = UIStroke:Clone()
UIStrokeTimeFreeze.Parent = TimeFreezeButton
...
-- Функция временной заморозки времени
TimeFreezeButton.MouseButton1Click:Connect(function()
    if not isTimeFrozen then
        isTimeFrozen = true
        TimeFreezeButton.Text = "Unfreeze Time"

        -- Заморозка всех игроков кроме текущего
        for _, player in pairs(game.Players:GetPlayers()) do
            if player ~= game.Players.LocalPlayer then
                local character = player.Character
                if character and character:FindFirstChild("HumanoidRootPart") then
                    character.HumanoidRootPart.Anchored = true
                    table.insert(frozenPlayers, character.HumanoidRootPart)
                end
            end
        end
    else
        ...
        -- Разморозка всех игроков кроме текущего
